...
public interface SceneRepository extends CrudRepository<Scene, Long> {
    ...
    Optional<Scene> findByScenarioAndContent(Scenario scenario, String content);
    Optional<Scene> findById(Long currentScenarioId);
    @Query("SELECT s.isEnding FROM Scene s WHERE s.id = :currentSceneId")
    Boolean findIsEndingById(@Param("currentSceneId") Long currentSceneId);
}
